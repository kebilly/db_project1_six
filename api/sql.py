...
class Member():
    def update_member(input):
        sql = 'UPDATE MEMBER SET NAME=:name, PASSWORD=:password, PHONE=:phone,EMAIL=:email WHERE MID=:mid'
        print(input)
        dd= DB.execute_input(DB.prepare(sql), input)
        print(dd)
        DB.commit()
    ...
    def create_member(input):
        sql = 'INSERT INTO MEMBER VALUES (null, :name, :account, :password, :identity,:phone,:email)'
...
class Product():
    ...
    def get_city(pid):
        sql = 'SELECT CITY FROM PRODUCT WHERE PID = :id'
        return DB.fetchone(DB.execute_input( DB.prepare(sql), {'id':pid}))[0]
    
    def get_product_byid1(pid):
        sql = 'SELECT CITY FROM PRODUCT WHERE PID = :id'
        return DB.fetchone(DB.execute_input( DB.prepare(sql), {'id':pid}))[0]
    
    def get_product_byid2(pid):
        sql = 'SELECT PDESC FROM PRODUCT WHERE PID = :id'
        return DB.fetchone(DB.execute_input( DB.prepare(sql), {'id':pid}))[0]
    
    def get_product_byid3(pid):
        sql = 'SELECT CATEGORY FROM PRODUCT WHERE PID = :id'
        return DB.fetchone(DB.execute_input( DB.prepare(sql), {'id':pid}))[0]

    def get_product_byid4(pid):
        sql = 'SELECT TIME FROM PRODUCT WHERE PID = :id'
        return DB.fetchone(DB.execute_input( DB.prepare(sql), {'id':pid}))[0]

    def check_name(pname):
        sql = 'SELECT COUNT(PNAME) FROM PRODUCT WHERE PNAME = :pname'
        result = DB.fetchone(DB.execute_input( DB.prepare(sql), {'pname':pname}))[0]
        if result == 0:
            return True
        else:
            return False
    ...
    def add_product(input):
        sql = 'INSERT INTO PRODUCT VALUES (:pid, :name, :price, :category, :description,:time,:city)'
    ...
    def update_product(input):
        sql = 'UPDATE PRODUCT SET PNAME=:name, PRICE=:price, CATEGORY=:category, PDESC=:description,TIME=:time,CITY=:city WHERE PID=:pid'
        DB.execute_input(DB.prepare(sql), input)
        DB.commit()
